...
import { button, input, span, table, tbody, td, tfoot, th, thead, tr } from "../../../jiffies/dom/html.ts";
...
export const history = FC(
	...
	(
		...
		const tryInput = ( /** @type import("../game/chess.js").Color */ color) => (
			/** @type Event */
			e,
		) => {
			notification.update("");
		...
		const body = tbody(
			...range(0, game.history.length, 2).map(
				(i) =>
					tr(
