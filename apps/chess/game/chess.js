   ...
   */
	findMove({ destination, rank, file, piece }) {
		...
		if (maybe.length === 1) {
			...
			return Err(
				`Ambiguous move: ${Pieces[piece] ?? ""}${file ?? ""}${rank ?? ""}${square(
					destination,
				)}`,
			);
		}
   ...
   */
	*findAll( /** @type {(i: number) => T} */ fn) {
	...
	moves( /** @type File */ file, /** @type Rank */ rank, calcThreat = false) {
		...
		const moveFilter = pieceType === PAWN
		...
		return moveHandler( /** @type Piece */ (pieceType), idx, moveFilter)
			.map((destination) => {
				const m = new Move({
					...
					rank,
					color,
					capture: this.board[destination] != EMPTY && Math.sign(
...
const FIDE_REGEX = /(?<move>\d+\.(?:\.\.)? )?(?:(?<piece>[RBNQK])?(?<file>[a-h])?(?<rank>[1-8])?(?<capture>x)?(?<destination>[a-h][1-8])(?<promotion>=?[RBNQK])?(?<check>[+#])?|(?<castle>O-O(?:-O)?)|(?<result>1-0|0-0|1\/2-1\/2))(?: \{(?<comment>[^}]*)}|)?/;
...
function moveHandler(
	...
	return MOVES[pieceType - 1].map(
		(ray) => [
			...takeWhile(filterFactory(), ray),
			...takeWhile(filterFactory(), ray.map((i) => -i)),
		],
	).flat().map((i) => idx + i);
}
...
function checkCheck( /** @type ChessGame */ board, /** @type Color */ color) {
	for (let i = 20; i < 100; i++) {
		...
		if (Math.abs(piece) === KING) {
			...
			if (color === pieceColor) {
				if (
					board.threat[asNum(file)][rank - 1][(-1 * color)].length > 0 /** @type Color */
				) {
					return true;
				}
			}
