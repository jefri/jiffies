	...
	figure(
		img({
			src: (avatar as ResumeTypes.ImageLink).link ? (
				avatar as ResumeTypes.ImageLink
			).link : `data:${(avatar as ResumeTypes.ImageData).mediaType};base64,${(
				avatar as ResumeTypes.ImageData
			).data}`,
		}),
	);
...
const projectDetail = ({ details }: ResumeTypes.ProjectExperience) =>
	details ? div(
		{ class: "project" },
		span(details.URL ? a({ href: details.URL }, details.name) : details.name),
