...
describe(
	...
	() => {
		it(
			"performs an operation using a context",
			() => {
		...
		it(
			"reports the result of a thrown error",
			() => {
				const context = TestContext();
		...
		it(
			"passes the context to the operation",
			() => {
				const op = using(
					TestContext,
					({ initialized, completed }) => ({ initialized, completed }),
				);
