...
export type Updater<E extends DOMElement> =
	& E
	& {
...
export type Updatable<E extends Element> =
	& E
	& {
		[Events]: Map<string, EventHandler>,
		update: (attrs?: DenormAttrs<E>, ...children: DenormChildren[]) => Node,
...
function isAttrs<E extends Element>(attrs: DenormAttrs<E> | undefined): attrs is Attrs<
	E
> {
	if (!attrs) {
		return false;
...
export function update<E extends DOMElement>(
	...
	Object
		.entries(events as NonNullable<typeof attrs.events>)
		.forEach(
			([k, v]) => {
	...
	if (_style) {
		if (typeof style === "string") {
			...
			Object
				.entries(style as Partial<CSSStyleDeclaration>)
				.forEach(
					([k, v]) => {
		...
		.forEach(
			([k, v]) => {
				if (k === "class" && typeof v === "string") {
					v.split(/\s+/m).filter((s) => s !== "").forEach(
						(c) => element.classList.add(c),
					);
